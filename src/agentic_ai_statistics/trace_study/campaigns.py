...
StudyStage = Literal["exploratory", "smoke", "main", "prediction"]
FOCUSED_SCENARIO = IncidentScenario.ORDERS_API_OUTAGE
FOCUSED_STRUCTURE = TaskStructure.RECOVERY
DEFAULT_RUNS: dict[StudyStage, int] = {"exploratory": 0, "smoke": 3, "main": 100, "prediction": 60}
...
def _write_scalar_artifacts(directory: Path, analysis: dict[str, Any]) -> None:
    ...
    _write_json(
        directory / "q01_data_dictionary.json",
        {
...
def _write_trajectory_artifact(directory: Path, analysis: dict[str, Any]) -> None:
    ...
    _write_json(
        directory / "q09_q14_trajectory_analysis.json",
        {
    ...
    _write_json(
        directory / "q02_scalar_distributions.json",
        {
            "schema_version": "12.0.0",
            "question_id": "Q02",
            "campaign_id": analysis["campaign_id"],
            ...
            "uncertainty": (
                "percentile bootstrap for means and medians; Wilson interval "
                "for success"
            ),
            "n_runs": analysis["n_runs"],
    ...
    _write_json(
        directory / "q03_batch_stability.json",
        {
            "schema_version": "12.0.0",
            "question_id": "Q03",
            "campaign_id": analysis["campaign_id"],
...
def _write_stochastic_artifact(directory: Path, analysis: dict[str, Any]) -> None:
    ...
    if process is not None:
        _write_json(
            directory / "q17_absorbing_process.json",
            {
                **process,
                "question_id": "Q17",
                "estimands": [
...
def analyze_prediction_campaign(training_campaign: Path, test_campaign: Path) -> dict[str, Any]:
    training_analysis = json.loads(
        (training_campaign / "analysis.json").read_text(encoding="utf-8")
    )
    test_analysis = json.loads((test_campaign / "analysis.json").read_text(encoding="utf-8"))
    result = evaluate_prediction(training_analysis, test_analysis)
    result["configuration_fingerprint_sha256"] = test_analysis.get(
        "configuration_fingerprint_sha256"
    )
    _write_json(test_campaign / "q18_held_out_trajectory_prediction.json", result)
    _write_json(
        test_campaign / "prediction_manifest.json",
        {
            "schema_version": "15.0.0",
            "training_campaign_id": result["training_campaign_id"],
            "test_campaign_id": result["test_campaign_id"],
            "new_model_calls_for_analysis": 0,
        },
    )
    return result
...
def analyze_collected_campaign(campaign_directory: Path) -> dict[str, Any]:
    ...
    analysis["configuration_fingerprint_sha256"] = manifest.get("configuration_fingerprint_sha256")
    scientific_cost = sum(detail.measurement.estimated_cost_usd for detail in details)
    excluded_cost = sum(detail.measurement.estimated_cost_usd for detail in excluded_details)
    ...
    analysis["excluded_provider_attempts"] = sum(
        detail.measurement.failure_type in PROVIDER_FAILURE_TYPES for detail in excluded_details
    )
...
def analyze_intervention_campaign(campaign_directory: Path) -> dict[str, Any]:
    ...
    primary = {
        ...
        "risk_difference_newcombe_95": newcombe_difference_interval(
            int(treatment["successes"]),
            int(treatment["assigned_runs"]),
            int(control["successes"]),
            int(control["assigned_runs"]),
        ),
        "fisher_exact_two_sided_p": fisher_exact_two_sided(
            int(treatment["successes"]),
            int(treatment["failures"]),
            int(control["successes"]),
            int(control["failures"]),
        )
        if treatment["assigned_runs"] and control["assigned_runs"]
        else None,
    }
    analysis.update(
        {
    ...
    _write_stochastic_artifact(campaign_directory, analysis)
    _write_json(
        campaign_directory / "q16_randomized_intervention.json",
        {
            "schema_version": "13.0.0",
            "question_id": "Q16",
...
def reanalyze_phase4(
    ...
    *,
    ...
    analysis, tables = analyze_details(details, campaign_id=campaign_id, study_stage="exploratory")
...
async def run_campaign(
    *,
    ...
    if manifest_path.is_file():
        ...
        campaign_directory.mkdir(parents=True, exist_ok=True)
        manifest = build_manifest(campaign_id, study_stage, planned_runs=planned_runs, mode=mode)
    ...
    accumulated_cost = sum(item.measurement.estimated_cost_usd for item in all_attempts)
    ...
    *,
    ...
    if manifest_path.is_file():
        ...
        expected = build_intervention_manifest(
            campaign_id,
            planned_runs=int(manifest["planned_runs"]),
            assignment_seed=int(manifest["assignment_seed"]),
            mode=mode,
        )
    ...
    schedule = cast(list[dict[str, Any]], manifest["schedule"])
    completed = [
        detail for detail in _load_details(campaign_directory) if is_scientific_observation(detail)
    ]
    completed_ids = {str(item.run_id) for item in completed}
    ...
    for item in schedule:
        ...
        detail = await run_incident(
            ...
            execution_order=int(item["execution_order"]),
            block=int(item["batch"]),
            intervention_arm=cast(Literal["control", "runbook_first"], item["intervention_arm"]),
        )
        accumulated_cost += detail.measurement.estimated_cost_usd
        if not is_scientific_observation(detail):
            _write_json(
...
def main() -> int:
    ...
    collect.add_argument("--stage", choices=["smoke", "main", "prediction"], required=True)
    ...
    collect.add_argument("--max-estimated-cost-usd", type=float, default=DEFAULT_COST_LIMIT_USD)
    ...
    predict.add_argument("--training-campaign", type=Path, required=True)
    predict.add_argument("--test-campaign", type=Path, required=True)
    ...
    if args.command == "reanalyze-phase4":
        ...
        result = analyze_prediction_campaign(args.training_campaign, args.test_campaign)
