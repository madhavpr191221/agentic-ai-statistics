...
export interface BehaviorMetadata {
  ...
  trace_steps: Array<{ sequence: number; tool_name: string; classification: 'oracle' | 'expected_rejection' | 'extra' | 'unexpected_rejection' | 'prohibited' }>
...
export interface BehaviorCampaignAnalysis {
  ...
  primary_model?: {
    formula: string
    primary: null | { model_type: string; coefficients: BehaviorCoefficient[]; pearson_dispersion: number }
    negative_binomial_sensitivity: null | { fitted: boolean; converged?: boolean; coefficients?: BehaviorCoefficient[]; error_type?: string; note?: string }
    note?: string
  }
  success_model?: {
    available: boolean
