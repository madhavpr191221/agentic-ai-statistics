  ...
  return <main className="workspace incident-workbench">
    <aside className="sidebar">
      <section className="panel control-panel">
        <p className="eyebrow">Single-run observation</p><h2>Investigate an incident</h2>
        ...
        <label>Execution mode<select value={mode} onChange={event => setMode(event.target.value as 'deterministic' | 'live')}>
          <option value="deterministic">Deterministic smoke test (no model spend)</option>
          <option value="live">Live model execution</option>
        </select></label>
        <p className="field-note">Each fresh run is one statistical observation. MCP calls and timings are measured at the local application boundary.</p>
        <button className="primary-button" disabled={busy} onClick={run}>{busy ? 'Running…' : 'Run observation'}</button>
    ...
    <div className="main-column">{!active ? <section className="panel welcome-panel"><p className="eyebrow">Start here</p><h2>Measure a real decision trace</h2><p>The agent gathers evidence through MCP, chooses a simulated action, and is scored against known ground truth.</p></section> : <>
      ...
      <section className="panel"><p className="eyebrow">Measured timing</p><h2>Total time and its recorded components</h2>
      ...
      <section className="panel"><p className="eyebrow">Observed trajectory</p><h2>{active.measurement.tool_sequence.length} ordered MCP calls</h2>
